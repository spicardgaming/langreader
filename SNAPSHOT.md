...
- Language picker in the new header (not present in the original mockup, added because it was explicitly requested) reuses the same Popular-group + divider + full-list pattern as the site-wide `Header.tsx` dropdown, restyled for this page — and pulls from the full 41-language list, matching the rest of the app.
